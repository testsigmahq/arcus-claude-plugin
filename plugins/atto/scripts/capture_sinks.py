...
- ``WebhookSink`` — POSTs each event to ``{chitragupt_host}/api/v1/plugin/events`` when the
  AuthState (populated by ``/atto:login``) carries a host.
"""
...
try:
    ...
    if os.environ.get("ATTO_DEBUG", "").strip() not in ("", "0", "false", "False"):
        print(
            "[atto] warning: fcntl unavailable (Windows?). Concurrent hooks may race "
...
def plugin_data_dir() -> str:
    ...
    return os.path.join(claude_home, "plugins", "data", "atto-inline")
...
def _log_webhook_failure(record: dict[str, Any], err: str, session_id: str) -> None:
    ...
    if raw in ("0", "false", "no", "off"):
        return
    if os.environ.get("ATTO_DEBUG", "").strip() in ("", "0", "false", "False"):
        return
    try:
        line = json.dumps(
            {
...
class WebhookSink:
    ...
    Host + account_id are read from the SSO config (``/atto:login``).
    ...
    def __init__(
        ...
        if not self._auth.usable():
            _log("WebhookSink: not authenticated, run /atto:login")
            self._url = None
    ...
    def handle_event(self, record: dict[str, Any]) -> None:
        ...
        token = self._auth.access_token()
        if token is None:
            _log("WebhookSink: skipping event (token unavailable; run /atto:login)")
            return
...
def build_default_sinks() -> ContextCaptureSink:
    ...
    each record to the ingest API when the AuthState (from ``/atto:login``) carries a
