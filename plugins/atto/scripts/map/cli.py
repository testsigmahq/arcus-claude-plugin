"""argparse entry point for /atto:map slash commands."""
...
def _cmd_ticket(args: argparse.Namespace) -> int:
    ...
    if not session_id:
        print("atto: no session manifest yet. Trigger any tool call first.", file=sys.stderr)
        return 2
    host = (auth.api_server or "").rstrip("/")
    token = auth.access_token()
    if not host or not token:
        print("atto: missing host or token.", file=sys.stderr)
        return 3
    ...
    try:
        ...
        print(f"atto: map ticket failed ({e.code}): {msg}", file=sys.stderr)
        return 4
    except (URLError, OSError, json.JSONDecodeError) as e:
        print(f"atto: map ticket error: {e}", file=sys.stderr)
        return 5
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="atto-map")
