...
- `/atto:map ticket <KEY>` — map ticket key to current session's workflow
...
2. If output is `(no project pinned; run /atto:project use <id>)`, ABORT. Tell the user to pin a project first via `/atto:project use <id>`. Do NOT execute the map command.
