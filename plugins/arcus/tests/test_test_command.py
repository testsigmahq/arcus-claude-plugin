# plugins/arcus/tests/test_test_command.py
import json
...
def test_version_bumped():
    # /arcus:test landed in 0.4.0; assert at least that (not an exact pin) so
...
def test_help_and_readme_mention_the_command():
    assert "/arcus:test" in (PLUGIN / "commands" / "help.md").read_text()
    assert "/arcus:test" in (REPO / "README.md").read_text()
